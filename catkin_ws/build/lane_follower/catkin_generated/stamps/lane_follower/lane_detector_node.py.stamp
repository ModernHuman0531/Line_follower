...
class LaneDetectorNode:
    ...
    def classify_lanes(self,frame, contours):
        ...
        # Use x coordinate to sort the contours, pick the most left and right lanes
        if valid_contours:
            contour_x_positions = []
            for contour in valid_contours:
                M = cv2.moments(contour)
                # If the area of contour is zero, skip it
                if M["m00"] != 0:      
                    # Use M["m10"] / M["m00"] to get the center x coordinate of the contour          
                    cx = int(M["m10"] / M["m00"])
                    contour_x_positions.append((contour, cx))

                # Sort the cntours by the x coordinate
                contour_x_positions.sort(key=lambda x: x[1])

                # Most left contours
                if contour_x_positions[0][1] < video_center:
                    left_lane = contour_x_positions[0][0]

                # Most right contours
                if len(contour_x_positions) > 1 and contour_x_positions[-1][1] > video_center:
                    right_lane = contour_x_positions[-1][0]
        ...
        if left_lane is not None and right_lane is not None:
            # Reshape the contours to a 2D array
            left_points = left_lane.reshape(-1, 2)
            right_points = right_lane.reshape(-1, 2)

            # Use argmax to find the largest y coordinate of the left and right lanes's position
            left_top = left_points[left_points[:, 1].argmax()]
            right_top = right_points[right_points[:, 1].argmax()]

            # Calculate the most bottom points of the left and right lanes
            left_bottom = left_points[left_points[:, 1].argmin()]
            right_bottom = right_points[right_points[:, 1].argmin()]

            # Use the four points to calculate the center of the lane
            center_top_x = (left_top[0] + right_top[0]) // 2
            center_bottom_x = (right_bottom[0] + left_bottom[0]) // 2

            # Draw the center lane
            cv2.line(result_image, (center_top_x, left_top[1]), (center_bottom_x, left_bottom[1]), (0, 0, 255), 2)
        ...
        if (right_lane is not None and left_lane is None) or (right_lane is None and left_lane is not None):
            cv2.line(result_image, (lane_center, height), (lane_center, height-50), (0, 0, 255), 2)
